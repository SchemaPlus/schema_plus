...
fixing the doc.  To use the prerelease, put this in your Gemfile:
    ...
    gem 'schema_plus', :git => "https://github.com/lomba/schema_plus.git" 
...
Base:
  ...
  column #unique_scope and #case_sensitive
...
SchemaPlus is an ActiveRecord extension that provides support for
...
SchemaPlus allows you to specify a foreign-key constraint for any column,
...
The above form is common enough that SchemaPlus defaults to automatically
...
SchemaPlus will by default create an index on each foreign key [TODO].
...
SchemaPlus supports and understands the following active-record
...
First you have to fetch sources from github as specs are not inculded in a gem.
  ...
  $ git clone https://dbsupport@github.com/dbsupport/schema_plus.git
  $ cd schema_plus
  $ bundle install
