...
describe "with multiple schemas" do
  ...
  before(:all) do
    newdb = case connection.adapter_name
            when /^mysql/i then      "CREATE SCHEMA IF NOT EXISTS schema_plus_index_test2"
            when /^postgresql/i then "CREATE SCHEMA schema_plus_index_test2"
            when /^sqlite/i then     "ATTACH ':memory:' AS schema_plus_index_test2"
            end
    begin
  ...
  before(:each) do
    ...
    connection.execute 'DROP TABLE IF EXISTS schema_plus_index_test2.users'
    connection.execute 'CREATE TABLE schema_plus_index_test2.users (id ' + case connection.adapter_name
  ...
  context "with indexes in each schema" do
    before(:each) do
      ...
      when /^mysql/i then      "index_users_on_login ON schema_plus_index_test2.users"
      when /^postgresql/i then "index_users_on_login ON schema_plus_index_test2.users"
      when /^sqlite/i then     "schema_plus_index_test2.index_users_on_login ON users"
      end + " (login)"
    end
