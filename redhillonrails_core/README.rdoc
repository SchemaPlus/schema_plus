...
=== Indexing using an arbitrary expression

PostgreSQL supports using creating indexes using arbitrary functions, such as <code>LOWER(name)</code>
(better served with the case-insensitive support), or <code>LOWER(SUBSTR(body, 1, 100))</code>. This
can even be used to create full text indexes:

  add_index :documents, :name => "index_documents_on_body_full_text", :expression => "gin (to_tsvector('english', body))"
...
Only the PostgreSQL adapter supports this feature.
